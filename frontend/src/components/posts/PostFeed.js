...
const PostFeed = ({ refreshFlag }) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/posts/${postToDelete.idPost}/`, {
        method: 'DELETE',
        credentials: 'include',
        headers: {
      ...
      if (res.status === 204) {
        ...
        setShowDeleteModal(false); // Cerrar modal aunque falle, mostrar error
      }
    } catch (err) {
            ...
            <>
              ...
              {soyDuenioDePost(post) && (
                <div style={{ marginTop: 10, display: 'flex', gap: 8 }}>
                  ...
                  <button style={{ color: 'white', background: 'red', border: 'none', borderRadius: 3, padding: '5px 12px' }} onClick={() => handleDeleteClick(post)}>Eliminar</button>
                </div>
              )}
            </>
          )}
