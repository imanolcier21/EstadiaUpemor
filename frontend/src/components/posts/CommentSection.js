...
const CommentSection = ({ idPost }) => {
  ...
  // Eliminar
  const handleDeleteClick = (idComentario) => {
    setCommentToDelete(idComentario);
  ...
  const confirmDelete = async () => {
    ...
    const csrftoken = getCookie('csrftoken');
    try {
      const res = await fetch(`/api/comentarios/${commentToDelete}/`, {
        method: 'DELETE',
        credentials: 'include',
        headers: {
          'X-CSRFToken': csrftoken
        ...
        <div key={comentario.idComentario} style={{ marginBottom: 10, borderBottom: '1px solid #eee', paddingBottom: 6 }}>
          ...
          {soyAutor(comentario) && editId !== comentario.idComentario && (
            <div style={{ display: 'flex', gap: 7, marginTop: 2 }}>
              ...
              <button onClick={() => handleDeleteClick(comentario.idComentario)} style={{ background: 'red', color: 'white', border: 'none', borderRadius: 3, padding: '2px 7px', fontSize: 13 }}>Eliminar</button>
            </div>
          )}
        </div>
      ))}
