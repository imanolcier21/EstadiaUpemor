    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (isRegistering) {
                    // REGISTRO EXITOSO: No loguear, mandar al login
                    setIsRegistering(false);
                    setMessage({ text: 'Registro exitoso. Por favor inicia sesión con tus nuevas credenciales.', type: 'success' });
                    setPassword('');
                    // No navegamos, nos quedamos en la misma pantalla (que ahora es Login)
                } else {
                    // LOGIN EXITOSO
                    login(response_data);
